...
let test_basic_of_string_valid () =
  ...
  in
  check bool "basic-black valid" true (is_valid "basic-black");
  check bool "basic-red valid" true (is_valid "basic-red");
  check bool "basic-green valid" true (is_valid "basic-green");
  check bool "basic-yellow valid" true (is_valid "basic-yellow");
  check bool "basic-blue valid" true (is_valid "basic-blue");
  check bool "basic-magenta valid" true (is_valid "basic-magenta");
  check bool "basic-cyan valid" true (is_valid "basic-cyan");
  check bool "basic-white valid" true (is_valid "basic-white");
  (* Bright variants *)
  check bool "bright-black valid" true (is_valid "bright-black");
  check bool "bright-red valid" true (is_valid "bright-red");
  check bool "bright-white valid" true (is_valid "bright-white")
...
let test_basic_of_string_case_insensitive () =
  ...
  let is_valid name =
    ...
    with InvalidColorName _ -> false
  in
  check bool "Basic-Black (mixed case)" true (is_valid "Basic-Black");
  check bool "BASIC-RED (all caps)" true (is_valid "BASIC-RED");
  check bool "Bright-White (title case)" true (is_valid "Bright-White")
...
let test_basic_nearest_exact () =
  ...
  check v4_testable "exact basic-red returns same"
    basic_red nearest
...
let test_basic_nearest_approximate () =
  ...
  let basic_red = Basic.to_color (Basic.of_string "basic-red") in
  check v4_testable "dark red -> basic-red"
    basic_red nearest;
  ...
  let basic_black = Basic.to_color (Basic.of_string "basic-black") in
  check v4_testable "very dark -> basic-black"
...
let test_xterm256_of_string_valid () =
  ...
  let is_valid name =
    ...
    with InvalidColorName _ -> false
  in
  check bool "black valid" true (is_valid "black");
  check bool "maroon valid" true (is_valid "maroon");
  check bool "green valid" true (is_valid "green");
  check bool "red valid" true (is_valid "red");
  (* Test a complex disambiguated name *)
  check bool "dark-olive-green-1a valid" true
...
let test_xterm256_of_string_case_insensitive () =
  ...
  let is_valid name =
    ...
    with InvalidColorName _ -> false
  in
  check bool "Red (title case)" true (is_valid "Red");
  check bool "BLACK (all caps)" true (is_valid "BLACK");
  check bool "Dark-Olive-Green-1a (mixed case)" true (is_valid "Dark-Olive-Green-1a")
...
let test_xterm256_to_code () =
  ...
  List.iter (fun color ->
      ...
      check bool (Printf.sprintf "%s code in range" color) true
        (code >= 0 && code <= 255)
...
let test_xterm256_nearest_exact () =
  ...
  check v4_testable "exact red returns same"
    red nearest
...
let test_xterm256_nearest_approximate () =
  ...
  let r = int_of_float (Float.round (255. *. Gg.Color.r c)) in
  let g = int_of_float (Float.round (255. *. Gg.Color.g c)) in
  check bool "orange-ish has high red" true (r > 200);
  check bool "orange-ish has medium-high green" true (g > 100);
  ...
  check bool "very dark maps to dark color" true
    (r < 20 && g < 20 && b < 20)
...
let test_xterm256_nearest_grayscale () =
  ...
  check bool "gray maps to neutral color" true
