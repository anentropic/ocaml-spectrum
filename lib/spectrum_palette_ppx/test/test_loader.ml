...
let test_load_valid_single () =
  with_temp_json valid_single_color_json (fun fname ->
      ...
      check int "loaded 1 color" 1 (List.length colors);
      let color = List.hd colors in
      check int "colorId = 32" 32 color.code;
      check string "name = BasicGreen" "BasicGreen" color.name;
      check int "r = 0" 0 color.r;
      check int "g = 128" 128 color.g;
      check int "b = 0" 0 color.b
    )

let test_load_valid_multiple () =
  with_temp_json valid_multiple_colors_json (fun fname ->
      let colors = load fname in
      check int "loaded 3 colors" 3 (List.length colors);
      (* Check first color *)
      let black = List.nth colors 0 in
      check int "black colorId = 30" 30 black.code;
      check string "black name" "BasicBlack" black.name;
      (* Check last color *)
      let white = List.nth colors 2 in
      check int "white colorId = 37" 37 white.code;
      check int "white r = 255" 255 white.r;
      check int "white g = 255" 255 white.g;
      check int "white b = 255" 255 white.b
    )

let test_load_assoc () =
  with_temp_json valid_multiple_colors_json (fun fname ->
      let assoc_list = load_assoc fname in
      check int "assoc list has 3 entries" 3 (List.length assoc_list);
      (* Check that colors are indexed by code *)
      let black = List.assoc 30 assoc_list in
      check string "code 30 -> BasicBlack" "BasicBlack" black.name;
      let red = List.assoc 31 assoc_list in
      check string "code 31 -> BasicRed" "BasicRed" red.name;
      let white = List.assoc 37 assoc_list in
      check string "code 37 -> BasicWhite" "BasicWhite" white.name
    )

let test_color_of_def () =
  let def = { name = "TestRed"; code = 99; r = 255; g = 0; b = 0 } in
  let color = color_of_def def in
  check v4_testable "TestRed converts to RGB(255,0,0)"
    (rgb 255 0 0) color;

  let def2 = { name = "TestGreen"; code = 100; r = 0; g = 255; b = 0 } in
  let color2 = color_of_def def2 in
  check v4_testable "TestGreen converts to RGB(0,255,0)"
    (rgb 0 255 0) color2;

  let def3 = { name = "TestGray"; code = 101; r = 128; g = 128; b = 128 } in
  let color3 = color_of_def def3 in
  check v4_testable "TestGray converts to RGB(128,128,128)"
...
let test_load_invalid_colorid_type () =
  check_raises "colorId wrong type"
...
let test_load_invalid_rgb_type () =
  check_raises "rgb wrong type (array instead of object)"
    (Failure "Invalid json data")
    (fun () ->
...
let test_load_not_array () =
  check_raises "top-level not array"
    (Failure "Invalid json data")
    (fun () ->
