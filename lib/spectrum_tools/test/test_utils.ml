...
let test_float_div () =
  check (float 0.001) "10 // 4 = 2.5" 2.5 (10 // 4);
  check (float 0.001) "1 // 3 = 0.333..." 0.333 (1 // 3);
  check (float 0.001) "255 // 2 = 127.5" 127.5 (255 // 2);
  check (float 0.001) "0 // 1 = 0.0" 0.0 (0 // 1)

let test_int_round () =
  (* Standard rounding: .5 rounds up, -.5 rounds down (away from zero) *)
  check int "0.5 -> 1" 1 (int_round 0.5);
  check int "1.5 -> 2" 2 (int_round 1.5);
  check int "2.5 -> 3" 3 (int_round 2.5);
  check int "3.5 -> 4" 4 (int_round 3.5);
  (* Negative rounding (away from zero) *)
  check int "-0.5 -> -1" (-1) (int_round (-0.5));
  check int "-1.5 -> -2" (-2) (int_round (-1.5));
  check int "-2.5 -> -3" (-3) (int_round (-2.5));
  (* Regular rounding *)
  check int "1.4 -> 1" 1 (int_round 1.4);
  check int "1.6 -> 2" 2 (int_round 1.6);
  check int "-1.4 -> -1" (-1) (int_round (-1.4));
  check int "-1.6 -> -2" (-2) (int_round (-1.6))

let test_clamp () =
  (* Within range *)
  check int "clamp 0 10 5 = 5" 5 (clamp 0 10 5);
  (* Below min *)
  check int "clamp 0 10 (-5) = 0" 0 (clamp 0 10 (-5));
  (* Above max *)
  check int "clamp 0 10 15 = 10" 10 (clamp 0 10 15);
  (* At boundaries *)
  check int "clamp 0 255 0 = 0" 0 (clamp 0 255 0);
  check int "clamp 0 255 255 = 255" 255 (clamp 0 255 255)

let test_min3_max3 () =
  check int "min3 5 3 7 = 3" 3 (min3 5 3 7);
  check int "min3 10 10 10 = 10" 10 (min3 10 10 10);
  check int "max3 5 3 7 = 7" 7 (max3 5 3 7);
  check int "max3 10 10 10 = 10" 10 (max3 10 10 10);
  check int "min3 (-5) 0 5 = -5" (-5) (min3 (-5) 0 5);
  check int "max3 (-5) 0 5 = 5" 5 (max3 (-5) 0 5)

let test_nearest_sqrt () =
  (* Floor-based sqrt *)
  check int "nearest_sqrt 16 = 4" 4 (nearest_sqrt 16);
  check int "nearest_sqrt 17 = 4" 4 (nearest_sqrt 17);
  check int "nearest_sqrt 25 = 5" 5 (nearest_sqrt 25);
  check int "nearest_sqrt 4 = 2" 2 (nearest_sqrt 4);
  check int "nearest_sqrt 1 = 1" 1 (nearest_sqrt 1);
  (* Round-based sqrt *)
  check int "nearest_sqrt' 16 = 4" 4 (nearest_sqrt' 16);
  check int "nearest_sqrt' 17 = 4" 4 (nearest_sqrt' 17);
  check int "nearest_sqrt' 18 = 4" 4 (nearest_sqrt' 18);
  check int "nearest_sqrt' 19 = 4" 4 (nearest_sqrt' 19);
  check int "nearest_sqrt' 20 = 4" 4 (nearest_sqrt' 20);
  check int "nearest_sqrt' 21 = 5" 5 (nearest_sqrt' 21)

let test_min_max_fold () =
  (* Test with non-empty lists *)
  check (option int) "min_fold [3;1;4;1;5] = Some 1"
    (Some 1) (min_fold [3; 1; 4; 1; 5]);
  check (option int) "max_fold [3;1;4;1;5] = Some 5"
    (Some 5) (max_fold [3; 1; 4; 1; 5]);

  (* Test with single element *)
  check (option int) "min_fold [42] = Some 42"
    (Some 42) (min_fold [42]);
  check (option int) "max_fold [42] = Some 42"
    (Some 42) (max_fold [42]);

  (* Test with empty list - now returns None instead of crashing *)
  check (option int) "min_fold [] = None"
    None (min_fold []);
  check (option int) "max_fold [] = None"
    None (max_fold [])
...
let test_of_rgb_to_rgba_roundtrip () =
  ...
  let test_color r g b =
    ...
    check int (Printf.sprintf "r=%d" r) r rgba.Rgba.r;
    check int (Printf.sprintf "g=%d" g) g rgba.Rgba.g;
    check int (Printf.sprintf "b=%d" b) b rgba.Rgba.b
  in
  test_color 255 0 0;
  test_color 0 255 0;
  test_color 0 0 255;
...
let test_map_color () =
  ...
  check (triple int int int) "map (*2) on red"
    (510, 0, 0) doubled;

  let gray_rgba = to_rgba gray_gg in
  let halved = map_color (fun x -> x / 2) gray_rgba in
  check (triple int int int) "map (/2) on gray"
    (64, 64, 64) halved

let test_map_color' () =
  let red_rgba' = to_rgba' red_gg in
  let doubled = map_color' (fun x -> x *. 2.0) red_rgba' in
  check (triple (float 0.01) (float 0.01) (float 0.01))
...
let test_map3 () =
  ...
  check (triple int int int) "map3 (*2) on (1,2,3)"
    (2, 4, 6) result;

  let result2 = map3 (fun x -> x + 10) (5, 10, 15) in
  check (triple int int int) "map3 (+10) on (5,10,15)"
    (15, 20, 25) result2
...
let test_product3 () =
  ...
  check (list (triple int int int)) "product3 [1;2] [3;4] [5;6]"
    expected result;

  (* Single element lists *)
  let result2 = product3 [1] [2] [3] in
  check (list (triple int int int)) "product3 single elements"
    [(1, 2, 3)] result2;

  (* Empty list produces empty result *)
  let result3 = product3 [] [1] [2] in
  check (list (triple int int int)) "product3 with empty list"
    [] result3

let test_range () =
  (* Basic range *)
  let r1 = range 5 |> List.of_seq in
  check (list int) "range 5" [0; 1; 2; 3; 4] r1;

  (* Range with from *)
  let r2 = range ~from:3 7 |> List.of_seq in
  check (list int) "range ~from:3 7" [3; 4; 5; 6] r2;

  (* Range with step *)
  let r3 = range ~step:2 10 |> List.of_seq in
  check (list int) "range ~step:2 10" [0; 2; 4; 6; 8] r3;

  (* Range with negative step *)
  let r4 = range ~from:10 ~step:(-2) 0 |> List.of_seq in
  check (list int) "range ~from:10 ~step:(-2) 0" [10; 8; 6; 4; 2] r4;

  (* Range with from and step *)
  let r5 = range ~from:5 ~step:3 15 |> List.of_seq in
  check (list int) "range ~from:5 ~step:3 15" [5; 8; 11; 14] r5;

  (* Empty range (from >= until with positive step) *)
  let r6 = range ~from:5 3 |> List.of_seq in
  check (list int) "range ~from:5 3 (empty)" [] r6

let test_range_invalid_step () =
  check_raises "range with step=0"
...
let test_adjacent_values_exn () =
  ...
  check (list int) "25 between 20 and 30" [20; 30] result;

  (* Invalid value (below all) -> raises *)
  check_raises "5 below all raises"
    (Invalid_argument "5")
    (fun () -> ignore (adjacent_values_exn set 5));

  (* Invalid value (above all) -> raises *)
  check_raises "60 above all raises"
    (Invalid_argument "60")
